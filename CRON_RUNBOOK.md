...
**來源數要求（時間 × 賽事階段雙維度，2026-07-07 優化）：**

以 `fixtures.json` 的 `kickoff_utc` 對比觸發時間判斷「36h 內/其他」；以 `stage` 欄位對應下表閾值：
...
- **來源須去重：同一域名只計算一個**（例如 bbc.com 5 篇只算 1 處），確保資訊增量。
- **8強（Quarter-final）起，額外要求必須包含以下三類高價值來源**（不足時優先補齊，質量優先於數量）：
  - ① **博彩隱含盤口**：Bet365、Pinnacle、William Hill、Betfair 等主流博彩公司的賠率，換算為隱含勝率（odds → probability）。
  - ② **統計/預測模型**：Opta、StatsBomb、FiveThirtyEight、Sofascore Prediction、ESPN FPI 等量化預測或統計數據。
  - ③ **官方傷兵/陣容報告**：FIFA 官網、各隊官方媒體頻道、UEFA、主流媒體（ESPN/Sky Sports/BBC）發佈的確認傷兵與首發名單消息。
  若某類來源當天無法找到，標注 `unavailable`，不計入來源數，但不影響預測繼續輸出。

（歷史變更：100/50 → 40/25 於 2026-06-23；40/25 → 純時間分級 20/12 於 2026-06-26；
2026-07-07 改為時間×階段雙維度，加入 8強起高價值來源類型要求。）
...
- `sources`（數量依「時間 × stage」分級表，含 url/type）、`source_count`
