...
## 7. 重新部署網站（僅 foreground 排程可用）
```bash
cd /home/user/workspace/wc2026/site && npm run build
```
然後 `deploy_website(project_path="/home/user/workspace/wc2026/site/dist/public", site_name="世界盃 2026 AI 預測中心", entry_point="index.html")`。
若曾發布到 pplx.app，使用相同 site_id 重新 `publish_website`。

## 8. 通知
- 若有新預測或新結果，發送 in-app 通知摘要（新預測場次、命中率變化）。
- 若無新事件，靜默結束。
