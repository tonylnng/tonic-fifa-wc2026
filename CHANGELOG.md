...
## [資料更新] 2026-06-21（HKT）預測批次 2026-06-20T2300Z（早間第 22 次）

### 新增（Added）

- **新預測**：
  - 第 41 場 阿根廷 2:1 奧地利（Group J，主勝，勝率 62/24/14，信心 0.72，88 處來源）＋ 7 個第三方 AI 對照（共識 2:0，H0.676/D0.204/A0.120）。
  - 第 42 場 法國 4:0 伊拉克（Group I，主勝，勝率 88/8/4，信心 0.87，100 處來源）＋ 7 個第三方 AI 對照（共識 3:0，H0.864/D0.093/A0.042）。
- **新結果**：
  - 第 33 場 荷蘭 5:1 瑞典（主勝，預測 2:1，勝負命中、比分未中）。
  - 第 34 場 德國 2:1 象牙海岸（主勝，預測 2:1，**比分精準命中**，溫達夫補時絕殺）。
- **賽後覆盤**：新增 #33、#34 兩場繁中覆盤。
- **球員統計**：新增荷蘭布羅貝（2 球）、加克波（2 球）、蘇默維爾（1 球）；瑞典埃蘭加（1 球）及 3 張黃牌；象牙海岸凱西耶（1 球）、德國溫達夫（2 球）；射手榜 77 人、紀律榜 85 人。

### 變更（Changed）

- 命中率：已評估 30 場，勝負命中 18/30（60.0%），比分精準 3/30（10.0%）。
- 校準：Brier 0.238、ECE 0.0873、過度自信 0.0287（校準較上次改善）。
- 基準分數：本站主預測在 30 場上勝負 18/30、比分 3/30。

---

## [功能更新] 2026-06-20（HKT）淘汰賽點球大戰維度（自 32 強起）

### 新增（Added）

- **淘汰賽點球風險分析**：自 32 強起，所有淘汰賽即將開賽比賽的預測檔加入 `prediction.knockout` 區塊，內含雙方近 5 年大型國際賽事點球大戰戰績、主罰球員心理素質、加時／點球／晉級機率與 `penalty_risk`／`risk_note`（全繁中，數據以權威來源為準不杜撰）。
- **網站點球風險提示**：預測卡片標頭新增「點球風險」徽章（依風險高低分色），詳情對話框新增「淘汰賽點球風險分析」區塊，呈現雙方點球大戰歷史、關鍵主罰者與晉級情境，於淘汰賽階段提供比小組賽更深入的報告，降低因和局引發的失準風險。小組賽不顯示此區塊。

### 變更（Changed）

- `data/predictions/SCHEMA.md`：新增 knockout 區塊定義（32 強起適用）。
- `site/client/src/lib/types.ts`：新增 `KnockoutInfo`、`PenaltyTeamHistory`、`PenaltyTaker` 型別；`prediction.knockout` 設為選用欄位。
- `site/client/src/components/PredictionCard.tsx`：新增 `PenaltyRiskBadge` 與 `KnockoutBlock` 元件，僅在 `prediction.knockout` 存在時渲染。
- 排程說明補充淘汰賽點球研究步驟（`CRON_RUNBOOK.md` §2.6、§1 階段判斷、§4 逐場自動維護；技能 runbook 同步）。

---

## [資料更新] 2026-06-20（HKT）預測批次 2026-06-20T1103Z（晚間第 21 次）

### 新增（Added）

- **新預測**：第 40 場 紐西蘭 0:2 埃及（客勝，勝率 19/27/54，信心 0.62，98 處來源）＋ 7 個第三方 AI 對照（共識 0:2，H0.191/D0.263/A0.546）。
- **新結果**：第 31 場 巴西 3:0 海地（主勝，比分精準命中，預測信心 0.82）；第 32 場 土耳其 0:1 巴拉圭（客勝，預測為 1:0 主勝，失準）。
- **賽後覆盤**：新增 #31、#32 兩場繁中覆盤。
- **球員統計**：新增 BRA-cunha（2 球）、BRA-vinicius（1 球）、PAR-galarza（1 球、1 黃）、PAR-almiron（1 紅）及多張黃牌；射手榜 73 人、紀律榜 82 人。

### 變更（Changed）

- 命中率：已評估 28 場，勝負命中 16/28（57.14%），比分精準 2/28（7.14%）。
- 校準：Brier 0.2429、ECE 0.1186、過度自信 0.0557。
- 基準分數：13 個來源、28 場；本站主預測勝負 16/28、比分 2/28。
...
- **基準排行榜可展開逐場明細**：在「校準與基準」頁，點擊任一 AI 大模型列（本站主預測 + 7 個第三方 AI）即可展開該模型的逐場命中表，欄位含 場次 / 對戰（繁中隊名）/ 預測比分 / 實際比分 / 勝負命中 / 比分命中。勝負命中以綠色 ✓、未中以紅色 ✗ 標示；比分命中以標靶圖示並整列高亮，一眼看出「勝負命中與比分命中分別是哪些賽事」。預設收合。
- **停更標籤**：博彩隱含機率、Opta 超級電腦、Polymarket、Kalshi、Football Meister AI 等早期手動寫入、後續未再供應資料的公開基準線，於排行榜加註「已停更·止於 #N」琥珀色標籤，並在頁尾說明其樣本有限僅供參考。

### 變更（Changed）

- `compute_benchmark_scores.py`：每個來源除彙總外，另輸出 `matches[]` 逐場明細（含繁中隊名、預測/實際比分、勝負/比分命中旗標），並依「非 AI 來源且最後一場落後當前最大已完成場次 ≥ 8 場」判定 `stale` 與 `latest_match`；新增 `max_match_completed` 欄位。內建隊名繁中對照表。
- 前端 `CalibrationTab.tsx` 重構排行榜為可展開列（`Fragment` + 逐場明細子表）；`types.ts` 新增 `BenchmarkMatchDetail` 型別與 `matches`/`stale`/`latest_match`/`max_match_completed` 欄位。
