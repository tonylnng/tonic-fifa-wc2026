...
排程整合：每輪對每場「即將開賽」比賽，在 Opus 主預測寫檔後呼叫本腳本。
"""
...
def build_consensus(ai_items, main_pred):
    ...
    logic = (
        f"綜合 {n_models} 個模型（本站 Opus 4.8 + {len(ai_items)} 家第三方）：" 
        f"勝率採主預測加權平均、比分採多數決"
...
def main():
    ...
    # 主預測（本站 Opus 4.8）位於 d["prediction"]；共識計算用。
    ...
    consensus = build_consensus(ai_items, main_pred)
